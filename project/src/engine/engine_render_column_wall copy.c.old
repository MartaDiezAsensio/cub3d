...
/*   Updated: 2024/10/02 20:50:40 by greus-ro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int get_point_of_wall(t_texture texture, t_dda_raycasting dda)
{
	...
	if(dda.side == 0 && dda.ray.x > 0) texX = texture.width - texX - 1;
	if(dda.side == 1 && dda.ray.y < 0) texX = texture.width - texX - 1;
	return (texX);
}
				 ...
				 t_dda_raycasting dda)
{
	...
	while(j <= render_col.floor_start)
	{
		...
		//printf("pixel = %d\n", wall_color);
		//mlx_put_pixel(engine.img, render_col.column, render_col.ceiling_end + j, wall_color);		
		mlx_put_pixel(engine.img, render_col.column, j, wall_color);		
		j++;	}
	return (true);
}
